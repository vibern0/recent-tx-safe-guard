...
import type { VerifiedDependency } from "../config/deployments";
...
export function createPasskeySigner(options: PasskeySignerOptions): SafeSigner {
  if (
    options.verifier.evidence !== "verified" ||
    !options.verifier.runtimeCodeHash ||
    !/^0x[0-9a-f]{64}$/i.test(options.verifier.runtimeCodeHash) ||
    options.verifier.address.toLowerCase() !== options.address.toLowerCase()
  ...
  return Object.freeze({
    ...
    sign: async (input) => {
      ...
      const result = await options.provider.request({ method: "eth_call", params: [{ to: options.verifier.address, data: encodeFunctionData({ abi: ERC1271_ABI, functionName: "isValidSignature", args: [request.safeTxHash, rawSignature] }) }, "latest"] });
      await assertProviderChain(options.provider, request.chainId);
