...
Codacy also reported `@openzeppelin/contracts@3.4.2-solc-0.7` under `@uniswap/v3-periphery`. This is not an unpatched vulnerable version for CVE-2021-39167: OpenZeppelin's advisory explicitly lists `3.4.2-solc-0.7` as patched (the same exact installed release). The finding is therefore a scanner false positive and does not justify replacing the Solidity-0.7 compatibility package with a different major/minor that could break the dependent package. Other lower-severity `uuid` and `bn.js` findings are outside this critical/high remediation scope.
