...
Codacy also reports transitive `elliptic`, `ws`, `uuid`, and `bn.js` versions from pinned Hardhat, Ethers, Safe, Zodiac, and coverage tooling. They are not direct runtime dependencies of the vault. No override was applied: forcing versions across these parents could break the pinned toolchain or invalidate the reviewed dependency boundary. Revisit them with coordinated upstream upgrades. The reported nested Uniswap/OpenZeppelin 3.x path is not present in this checkout's dependency graph.
