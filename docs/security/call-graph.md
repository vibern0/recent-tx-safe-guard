...
The invariant suite inventories every public authorization surface, requires each to appear in this call graph, rejects public spend primitives, and checks explicit fail-closed branches for signatures, queues, calls, and recovery. Imported Safe and Zodiac code is not represented as this repository's audit. Slither is filtered to repository security code and excludes legacy/test support; findings remain a required human-review input.
